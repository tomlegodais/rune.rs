...
use crate::{
    content::{CombatTarget, start_combat},
    player::{
...
pub fn dispatch(
    ...
    if op == Op::OpT {
        return match target {
            InteractionTarget::Npc { index } => Some(start_combat(CombatTarget::Npc(index))),
            InteractionTarget::Player { index } => Some(start_combat(CombatTarget::Player(index))),
            _ => None,
        };
    }
